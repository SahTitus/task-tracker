...
export const TaskForm = ({ addTask, taskToEdit, setTaskToEdit, editTask, toggleForm }) => {
	...
	return (
		<div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
			<div className="w-full max-w-lg p-6 bg-white rounded-xl shadow-lg transform transition-all duration-300 sm:max-w-xl">
				<h2 className="text-xl font-semibold mb-6 text-center text-gray-800">Add New Task</h2>
				<form className="space-y-5" onSubmit={handleSubmit}>
					...
					<div>
						<label className="block text-sm font-medium text-gray-600 text-left">Due Date</label>
						<input
							type="date"
							name="dueDate"
							value={task.dueDate}
					...
					<div>
						<label className="block text-sm font-medium text-gray-600 text-left">Status</label>
						<select
							name="status"
							value={task.status}
							onChange={handleChange}
					...
					{task.status === 'in-progress' && (
						<div>
							...
							<input
								...
								min="0"
								max="100"
								value={task.status === 'completed' ? 100 : task.status === 'pending' ? 0 : task.progress}
								onChange={handleChange}
