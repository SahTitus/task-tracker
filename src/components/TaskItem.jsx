...
export const TaskItem = ({ task, handleOnEditClick, deleteTask, toggleCheckTask }) => {
	const { id, title, checked, description, dueDate, progress, status } = task;
	...
	return (
		<div
			className={`group relative overflow-hidden shadow-md rounded-2xl transition-all duration-300 ${checked ? 'bg-slate-50/80 backdrop-blur-sm' : 'bg-white hover:shadow-lg hover:shadow-slate-200/50 hover:-translate-y-0.5'} border border-slate-100 hover:border-slate-200`}
		...
		>
			...
			<div className="relative h-1.5 overflow-hidden">
				...
				<div
					...
					style={{ width: `${progress}%`, transform: `scaleX(${isHovered ? 1.03 : 1})` }}
				/>
			</div>
			...
			<div className="px-5 py-4">
				<div className="flex items-start gap-4">
					<Button
						type="button"
						label=""
						arialLabel={`${checked ? "Uncheck task" : "Check task"}`}
						isDropdown={true}
						onClick={() => toggleCheckTask(id)}
						icon={
							<>
								<span className={`absolute inset-0 rounded-full ring-2 ring-slate-200 transition-all duration-300 ${checked ? 'scale-0 opacity-0' : 'hover:ring-green-400 hover:ring-offset-2'}`} />
								{checked ? <FaCircleCheck className="size-full p-1.5" /> : <BiCircle className="size-full p-1" />}
							</>
						}
						styles={`relative flex-shrink-0 rounded-full size-6 transition-all duration-300 ${checked ? 'bg-gradient-to-br from-green-400 to-green-500 text-white scale-100' : 'text-slate-300 hover:text-green-500 hover:scale-105'}`}
					/>

					<div className="flex-1 min-w-0">
						<h2 className={`text-start text-base font-semibold leading-tight transition-colors duration-200 ${isExpanded ? 'hidden sm:block' : 'block'} ${checked ? 'text-slate-400 line-through' : 'text-slate-700'} overflow-hidden whitespace-nowrap text-ellipsis`}>
							{title}
						</h2>
				...
				<div className="relative mt-4 flex items-center gap-2 justify-between">
					<span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium ${currentStatus.bgStyles} transition-all duration-200 hover:shadow-md hover:scale-105`}>
						{currentStatus.icon}
...
const ActionButtons = ({ deleteTask, handleOnEditClick, isMenuOpen, handleMenuToggle }) => {
	return (
...
const ExpandedDetails = ({ description, title, dueDate, checked, progress }) => {
	return (
		<div className="overflow-hidden">
			<div className="mt-4 space-y-3">
				<h2 className={`block sm:hidden text-start text-base font-semibold leading-tight transition-colors duration-200 ${checked ? 'text-slate-400 line-through' : 'text-slate-700'}`}>
					{title}
				</h2>
